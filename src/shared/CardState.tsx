...
export const CardState = ({ type='success', text }: TCardState) => (
    ...
    <Box sx={{padding:'3px'}} className={`${type === 'success' ? '-bg--daisy-state-success -text--daisy-state-full-success' : '-bg--daisy-state-error -text--daisy-state-full-error'} m-0 w-full rounded-xl`}>
        {text}
    </Box>
)
